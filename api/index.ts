...
const fructeExotice: Record<string, FruitInfo> = {
  'dragon fruit': {
    nume: 'Dragon Fruit (Pitaya)',
    origine: 'America Centrală și de Sud',
    descriere: 'Un fruct spectaculos cu coajă roz-violet și pulpă albă sau roșie cu semințe negre. Are un gust dulce și blând, similar cu kiwi și pere.',
...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // API endpoint pentru fruct individual
  const fructMatch = path.match(/\/api\/fruct\/(.+)/);
  if (fructMatch) {
    const numeCautat = decodeURIComponent(fructMatch[1]).toLowerCase().trim();

    // Căutare exactă
    let fruct = fructeExotice[numeCautat];

    // Căutare parțială în chei
    if (!fruct) {
    ...
    if (fruct) {
      ...
      res.json({
        success: false,
        message: `Fructul "${numeCautat}" nu a fost găsit.`,
  ...
  // API endpoint pentru toate fructele
  if (path === '/api/toate-fructele') {
    res.json({
      success: true,
      data: Object.values(fructeExotice)
    });
    return;
  }

  // Răspuns pentru rădăcină - returnează datele aplicației
  res.json({
    message: 'Exot API',
    endpoints: [
      '/api/fruct/:nume',
